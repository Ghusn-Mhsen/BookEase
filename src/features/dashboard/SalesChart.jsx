...
import { eachDayOfInterval, isSameDay, isValid, subDays } from "date-fns";
...
export default function SalesChart({ bookings, numDays }) {
  ...
  const data = allDays.map((date) => {
    const validBookings = bookings.filter((booking) => {
      ...
      return isValid(bookingDate) && isSameDay(date, bookingDate);
    });

    return {
      label: format("MMM dd", date),
      totalSales: validBookings.reduce((acc, cur) => acc + cur.totalPrice, 0),
      extrasSales: validBookings.reduce((acc, cur) => acc + cur.extraPrice, 0),
    };
  });
    ...
    <StyledSalesChart>
      ...
      <ResponsiveContainer width='100%' height={300} >
        <AreaChart data={data}>
          ...
          <Area dataKey='extrasSales' type='monotone' fill={colors.extrasSales.fill} stroke={colors.extrasSales.stroke} strokeWidth={2} name="Extras Sales" unit='$' />
        </AreaChart>
      </ResponsiveContainer>
    </StyledSalesChart>
  )
